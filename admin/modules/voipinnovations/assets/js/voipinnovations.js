$(function () {
  if (!Voipinnovations.trunk_groups) {
    $('.trunkgroup').removeClass('hidden')
  }
  updateInfoDisplay()

  voipinnovationsCookie = getCookie('voipinnovations')
  voipinnovationsToggle = voipinnovationsCookie !== undefined ? $.parseJSON(voipinnovationsCookie) : {}
  if (!$.isEmptyObject(voipinnovationsToggle)) {
    $.each(voipinnovationsToggle, function (section, state) {
      if (!state) {
        $('.' + section).hide()
...
$('#firewall-test-button').click(function () {
  ...
  $.post('ajax.php?module=voipinnovations&command=testFirewall')
    .done(function (data) {
      ...
      if (data.status == 'success') {
        ...
        $('#firewall_status').removeClass('firewall-fail')
        ...
        $('#firewall_status').addClass('firewall-fail')
...
function getextinfo(ext, did) {
  ...
  dat.ext = ext
  $.post(
    'ajax.php?module=voipinnovations&command=getextinfo',
    dat,
    function (data) {
      if (data.status) {
        if (data.outboundcid != '') {
...
function updateInfoDisplay(_callback) {
  if (Voipinnovations.status != 'valid') {
  ...
  $.get('ajax.php?module=voipinnovations&command=getAccountInfo')
      ...
      if (data.status == 'success') {
        if (data.message.message != '') {
          if (data.message.type == 'error') {
            errorMessage(data.message.message, _('Message from VoIP_Innovations'))
          } else {
            noticeMessage(data.message.message, _('Message from VoIP_Innovations'))
          }
        }
        ...
        if (data.trunk_qualify != undefined) {
          $.each(data.trunk_qualify, function (key) {
            ...
            if (ping == null) {
              $('#trunk_qualify_' + key)
                .val(this)
                .css('background-color', '#ba0000')
            } else {
              if (ping[1] < 1500) {
                $('#trunk_qualify_' + key)
                  .val('OK')
                  .css('background-color', '#00cc00')
              } else {
                $('#trunk_qualify_' + key)
                  .val('OK')
                  .css('background-color', '#eaff33')
              }
            }
          })
        }
...
function addRoutes() {
  var dat = {}
  dat.sip_username = Voipinnovations.sip_username ? Voipinnovations.sip_username : ''
  $.post(
    'ajax.php?module=voipinnovations&command=addroutes',
    dat,
    function (data) {
      if (data.status) {
        updateInfoDisplay()
...
$('.voipinnovations-section').click(function () {
  var section = $(this).attr('id')
  if ($('.' + section).is(':visible')) {
    $('.' + section).hide()
    $(this).removeClass('toggle-minus').addClass('toggle-plus')
    //set cookie of hidden section
    voipinnovationsToggle = $.parseJSON(getCookie('voipinnovationsToggle')) || {}
    voipinnovationsToggle[section] = false
    setCookie('voipinnovationsToggle', JSON.stringify(voipinnovationsToggle))
  } else {
    $('.' + section).show()
    $(this).removeClass('toggle-plus').addClass('toggle-minus')
    //set cookie of hidden section
    voipinnovationsToggle = $.parseJSON(getCookie('voipinnovationsToggle')) || {}
    if (voipinnovationsToggle.hasOwnProperty(section)) {
      voipinnovationsToggle[section] = true
      setCookie('voipinnovationsToggle', JSON.stringify(voipinnovationsToggle))
    }
  }
})
...
$(document).on('fpbx_reload', function (event, data) {
  if (data.complete && typeof data.errors.data === 'object' && !data.errors.data.status) {
    //location.href = "config.php?display=voipinnovations"
  }
})
...
Voipinnovations['trunk_groups'].forEach(function (elem) {
...
function updateCID(did, cid, ecid, _callback) {
  $.getJSON(
    'ajax.php',
    { module: 'voipinnovations', did: did, cid: cid, ecid: ecid, command: 'updateCID' },
    function (data) {
      toggle_reload_button('show')
      _callback(data)
    }
...
function updateFailover(did, dest, num, _callback) {
  $.getJSON(
    'ajax.php',
    { module: 'voipinnovations', did: did, dest: dest, num: num, command: 'updateFailover' },
    function (data) {
      _callback(data)
    }
  )
...
function updateDest(did, dest, _callback) {
  $.getJSON(
    'ajax.php',
    { module: 'voipinnovations', did: did, dest: dest, command: 'updateDest' },
    function (data) {
      toggle_reload_button('show')
      _callback(data)
    }
...
function updateAreaCode(areacode, _callback) {
  $.getJSON(
    'ajax.php',
    { module: 'voipinnovations', areacode: areacode, command: 'updateAreaCode' },
    function (data) {
      _callback(data)
    }
  )
...
function clearFailover(_callback) {
  $.getJSON('ajax.php', { module: 'voipinnovations', command: 'clearFailover' }, function (data) {
    _callback(data)
  })
}

function bind_did_table() {
  $.each(Voipinnovations.dids, function (key, did) {
...
function did_did_formatter(v, r) {
  var url = '?display=did&view=form&extdisplay=' + encodeURIComponent(r.did) + encodeURIComponent('/')
...
function route_gateway_formatter1(v, r) {
  ...
  var disabled = r['disabled']
  var html = ''
  html += '<span class="radioset">'
  html +=
    '<input type="radio" data-gw="1" data-route="' +
    r.id +
    '" name="route' +
    r.id +
    '_1" id="routeyes' +
...
function route_gateway_formatter2(v, r) {
  ...
  html += '<span class="radioset">'
  html +=
    '<input type="radio" data-gw="2" data-route="' +
    r.id +
    '" name="route' +
    r.id +
    '_2" id="routeyes' +
...
$('#ssroutes').on('post-body.bs.table', function () {
  $('input[type=radio][name^=route]').change(function (e) {
    ...
    $.getJSON(
      'ajax.php',
      { module: 'voipinnovations', action: command, route: route, command: 'updateroute' },
      function (data) {
...
function failoverFormatter(v, r) {
  if (Voipinnovations.account_type == 'TRIAL') {
  ...
  return (
    '<a href="#"  class="failoverlink" data-dest="' +
    (v.dest ? v.dest : '') +
    '" data-num="' +
    (v.num ? v.num : '') +
    '" data-did="' +
...
function destinationFormatter(v, r) {
  ...
  return (
    '<a href="#"  class="destlink" data-dest="' +
    ddest +
    '" data-did="' +
    r.did +
    '"><i class="fa fa-edit"></i> ' +
...
function e911Formatter(v, r) {
  ...
  return (
    `<a href="#" class="e911link" data-did="${r.did}" data-name="${v.name}" data-street1="${v.street1}" data-street2="${v.street2}" data-city="${v.city}" data-state="${v.state}" data-zip="${v.zip}"><i class="fa fa-edit"></i>` +
    _('Edit/Update') +
    '</a>'
  )
}
function cidFormatter(v, r) {
  ...
  return (
    `<a href="#" class="cidlink" data-did="${r.did}" data-cid="${r.cid}" data-ecid="${r.ecid}"><i class="fa fa-edit"></i>` +
    _('Edit/Update') +
    '</a>'
  )
}
