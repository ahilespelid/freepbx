    ...
    return function errorHandlerMiddleware(err, req, res, next) {
        ...
        if (err.failedValidation) {
            ...
            response.validationErrors = errors
                ? errors.map(error => parseSwaggerError(error))
                : [err.message];
        } else if (err.isJoi) {
            // special handling for joi validation errors (bookshelf most likely)
            response.message = 'Validation failed';
            res.statusCode = 400;

            const errors = _.get(err, 'details', [err]);
            response.validationErrors = errors.map(error => error.message);
        } else if (err.providerErrors) {
            // special handling for provider errors (bandwidth)
            response.message = 'Provider error';
            response.providerErrors = err.providerErrors;
             ...
             * }
             */
            response.message = _.get(err, 'error.message', 'API error');
            response.apiErrors = [];
            if (err.error) {
         ...
         */
        response.sourceService = _.get(err, 'error.sourceService', err.sourceService || serviceName);
