...
if(FreePBX::Modules()->checkStatus("sipstation",MODULE_STATUS_NEEDUPGRADE) && FreePBX::Modules()->checkStatus('sms') && FreePBX::Modules()->checkStatus('userman') && FreePBX::Modules()->checkStatus('ucp')) {
	$info = FreePBX::Modules()->getInfo("sipstation");
	if(version_compare_freepbx($info['sipstation']['dbversion'], "13.0.13.5", "<")) {
		$users = FreePBX::Userman()->getAllUsers();
		foreach($users as $user) {
			$dids = FreePBX::Sms()->getAssignedDIDs($user['id']);
			if(!empty($dids)) {
				FreePBX::Ucp()->setSettingByID($user['id'],'Sipstation','sms_enable',true);
			}
		}
	}
}
...
//SS_API_URL
$set['value'] = 'https://push2.schmoozecom.com/sipstation';
$set['defaultval'] =& $set['value'];
$set['readonly'] = 1;
$set['hidden'] = 1;
$set['level'] = 1;
$set['module'] = 'sipstation';
$set['category'] = 'SIPStation Module';
$set['emptyok'] = 0;
$set['sortorder'] = 1;
$set['name'] = 'Sets the api url for sipstation this should never be touched';
$set['description'] = 'This will reset on every install of SIPStation';
$set['type'] = CONF_TYPE_TEXT;
$freepbx_conf->define_conf_setting('SS_API_URL',$set);
$freepbx_conf->reset_conf_settings(array('SS_API_URL'),true);
...
$set['defaultval'] =& $set['value'];
$set['options'] = '';
$set['name'] = _('Enable Experimental SIPStation PJSIP Mode');
$set['description'] = _('When enabled, the SIPStation module will auto generate PJSIP trunks, otherwise chan_sip trunks are created. Chan_sip is deprecated and not recommended.');
$set['emptyok'] = 0;
$set['level'] = 2;
$set['readonly'] = 0;
$set['hidden'] = 0;
$set['type'] = CONF_TYPE_BOOL;
$freepbx_conf->define_conf_setting('ENABLE_SS_PJSIP',$set);
